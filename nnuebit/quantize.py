...
def quantize(file: str, out: str | None = None) -> None:
    ...
    with open(out, 'wb') as f:
        ...
        print('% 5d <=    ft_biases    <= % 5d absolute mean: % 5d' % (torch.min(tensor).item(), torch.max(tensor).item(), mean))
        ...
        tensor = tensor.round().long()
        print('% 5d <=    ft_weights   <= % 5d absolute mean: % 5d' % (torch.min(tensor[:, :256]).item(), torch.max(tensor[:, :256]).item(), mean))
        bytes = tensor.detach().numpy().astype('<u2').tobytes()
        f.write(bytes)
        ...
        mean = torch.mean(torch.abs(tensor)).round().long().item()
        tensor = tensor.round().long()
        print('% 5d <= hidden1_biases  <= % 5d absolute mean: % 5d' % (torch.min(tensor).item(), torch.max(tensor).item(), mean))
        ...
        mean = torch.mean(torch.abs(tensor)).round().long().item()
        tensor = tensor.round().long()
        print('% 5d <= hidden1_weights <= % 5d absolute mean: % 5d' % (torch.min(tensor).item(), torch.max(tensor).item(), mean))
        ...
        mean = torch.mean(torch.abs(tensor)).round().long().item()
        tensor = tensor.round().long()
        print('% 5d <= hidden2_biases  <= % 5d absolute mean: % 5d' % (torch.min(tensor).item(), torch.max(tensor).item(), mean))
        bytes = tensor.detach().numpy().astype('<u4').tobytes()
        f.write(bytes)
        ...
        mean = torch.mean(torch.abs(tensor)).round().long().item()
        tensor = tensor.round().long()
        print('% 5d <= hidden2_weights <= % 5d absolute mean: % 5d' % (torch.min(tensor).item(), torch.max(tensor).item(), mean))
        bytes = tensor.detach().numpy().astype('<u1').tobytes()
        f.write(bytes)
        ...
        mean = torch.mean(torch.abs(tensor)).round().long().item()
        tensor = tensor.round().long()
        print('% 5d <=  output_biases  <= % 5d absolute mean: % 5d' % (torch.min(tensor).item(), torch.max(tensor).item(), mean))
        bytes = tensor.detach().numpy().astype('<u4').tobytes()
        f.write(bytes)
        ...
        mean = torch.mean(torch.abs(tensor)).round().long().item()
        tensor = tensor.round().long()
        print('% 5d <=  output_weights <= % 5d absolute mean: % 5d' % (torch.min(tensor).item(), torch.max(tensor).item(), mean))
        bytes = tensor.detach().numpy().astype('<u1').tobytes()
        f.write(bytes)
