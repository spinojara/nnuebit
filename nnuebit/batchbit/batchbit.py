...
class batch(ctypes.Structure):
    ...
    def get_tensors(self, device):
        ...
        f1 = torch.sparse_coo_tensor(ind1, val1, (self.size, model.FT_IN_DIMS + model.VIRTUAL), check_invariants=False).to(device=device, non_blocking=True)
        f2 = torch.sparse_coo_tensor(ind2, val2, (self.size, model.FT_IN_DIMS + model.VIRTUAL), check_invariants=False).to(device=device, non_blocking=True)

        f1._coalesced_(True)
        f2._coalesced_(True)

        return bucket, f1, f2, eval
